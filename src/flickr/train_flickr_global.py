...
def main(config_path: str):
    ...
    experiment_name = mlflow_cfg.get("experiment_name", "flickr-global")
    ...
    run_dir = output_root / paths.get("run_name", "global_run")
    ...
    production_df = manifest[manifest["split"] == "production"].reset_index(drop=True)
    ...
    prod_ds = GlobalEmbeddingDataset(production_df, cache_dir)
    ...
    history = []
    best_val_srcc = -1e9
    best_ckpt = ckpt_dir / "best_global_model.pth"
    last_ckpt = ckpt_dir / "last_global_model.pth"
    ...
    mlflow.log_metrics(
        {
            ...
            "num_production_rows": len(production_df),
        },
        step=0,
    )
    ...
    mlflow.log_metrics(
        {
            ...
            "production_mse": prod_metrics["mse"],
            "production_mae": prod_metrics["mae"],
            "production_plcc": prod_metrics["plcc"],
            "production_srcc": prod_metrics["srcc"],
        }
    )
    ...
    mlflow.log_artifact(str(prod_pred_csv))
...
if __name__ == "__main__":
    config_path = sys.argv[1] if len(sys.argv) > 1 else "configs/train_global.yaml"
    main(config_path)